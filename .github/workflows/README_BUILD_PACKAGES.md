...
This document describes the GitHub Actions workflow for building relocatable ROCm Bandwidth Test (RBT) packages using the ROCm SDK from [TheRock](https://github.com/ROCm/TheRock).
...
- Push to `amd-staging`, `amd-mainline`, or `release/**` branches
- Pull requests to `amd-staging` or `amd-staging` branches
...
└── Execute build_packages_local.sh
    ...
    │         -DCMAKE_INSTALL_RPATH="$ORIGIN:$ORIGIN/../lib:$ORIGIN/../lib/rbt:/opt/rocm/extras-$ROCM_MAJOR/lib" \
...
```bash
CMAKE_INSTALL_RPATH="$ORIGIN:$ORIGIN/../lib:$ORIGIN/../lib/rbt:/opt/rocm/extras-<ROCM_MAJOR>/lib"
```
...
**Dynamic ROCm Path Discovery**: `FETCH_ROCMPATH_FROM_ROCMCORE=ON` allows RBT to automatically detect ROCm installation location at runtime from ROCm core libraries.
...
5. **Upload to S3** (when repo is `ROCm/rocm_bandwidth_test`) – Each job uploads its packages to S3 using OIDC. The bash routing logic determines the S3 path: `release/*` branch builds (push or manual) go to `release/`, scheduled/push/manual builds go to `nightly/`, and PR builds go to a ref-specific path. Requires `AWS_S3_BUCKET` (variable) and `AWS_ROLE_ARN` (secret). Skipped gracefully if `AWS_S3_BUCKET` is not set.
...
1. **Repository secret** (Secrets tab, see above):
   - Name: `AWS_ROLE_ARN`
   - Value: the IAM role ARN to assume for S3 upload (e.g. `arn:aws:iam::123456789012:role/rocm-bandwidth-test-s3-upload-role`). Keeps the role ID hidden from the workflow.
...
```
s3://<bucket>/nightly/rbt/
├── deb/
│   ├── amdrocm7-rocm_bandwidth_test_2.6.2-r0711.20260423_amd64.deb
│   ├── Packages
│   ├── Packages.gz
│   └── Release
├── rpm/
│   ├── amdrocm7-rocm_bandwidth_test_2.6.2-r0711.20260423.x86_64.rpm
│   └── repodata/
│       ├── repomd.xml
│       ├── primary.xml.gz
│       ├── filelists.xml.gz
│       └── other.xml.gz
└── tar/
    └── amdrocm7-rocm_bandwidth_test_2.6.2-r0711.20260423-Linux.tar.gz
```
...
echo "deb [trusted=yes] https://<bucket>.s3.amazonaws.com/nightly/rbt/deb/ ./" \
  | sudo tee /etc/apt/sources.list.d/rbt-nightly.list

# Or the release repo
echo "deb [trusted=yes] https://<bucket>.s3.amazonaws.com/release/rbt/deb/ ./" \
  | sudo tee /etc/apt/sources.list.d/rbt-release.list

sudo apt update
sudo apt install amdrocm7-rocm-bandwidth-test  # Replace 7 with your ROCm major version
```
...
```bash
# Add the nightly repo (replace <bucket> with the actual S3 bucket name)
cat <<'EOF' | sudo tee /etc/yum.repos.d/rbt-nightly.repo
[rbt-nightly]
name=RBT Nightly Packages
baseurl=https://<bucket>.s3.amazonaws.com/nightly/rbt/rpm/
enabled=1
gpgcheck=0
EOF

# Or the release repo
cat <<'EOF' | sudo tee /etc/yum.repos.d/rbt-release.repo
[rbt-release]
name=RBT Release Packages
baseurl=https://<bucket>.s3.amazonaws.com/release/rbt/rpm/
enabled=1
gpgcheck=0
EOF

sudo yum install amdrocm7-rocm-bandwidth-test  # Replace 7 with your ROCm major version
# or: sudo dnf install amdrocm7-rocm-bandwidth-test
```
...
- **Building**: Compiles RBT with parallel builds
...
| `ROCM_LIBPATCH_VERSION` | Auto-extracted from `ROCM_VERSION` | Major.minor in xxyy format with zero padding (e.g., `7.11` → `0711`, `8.0` → `0800`) - used for RBT version tagging |
...
Packages are named automatically by CPack using the RBT version from `CMakeLists.txt`:

- **DEB**: `amdrocm<ROCM_MAJOR>-rocm-bandwidth-test_${RBT_VERSION}_amd64.deb`
- **RPM**: `amdrocm<ROCM_MAJOR>-rocm-bandwidth-test-${RBT_VERSION}.x86_64.rpm`
- **TGZ**: `amdrocm<ROCM_MAJOR>-rocm-bandwidth-test-<RBT_VERSION>-<PACKAGE_RELEASE>-Linux.tar.gz` (CPack: same **release** suffix as DEB/RPM, e.g. `r0711.20260423` from `ROCM_LIBPATCH_VERSION` and date, or a PR-specific suffix)

The **patch version** in `RBT_VERSION` is automatically computed from the number of commits since the last `v<major>.<minor>.*` git tag. For example, with tag `v1.3.0` and 15 commits since, and a release of `r0711.20260423`:
```
amdrocm7-rocm_bandwidth_test_2.6.2_r0711.20260423_amd64.deb
amdrocm7-rocm_bandwidth_test_2.6.2-r0711.20260423.el8.x86_64.rpm
amdrocm7-rocm_bandwidth_test-2.6.2-r0711.20260423-Linux.tar.gz
```
...
sudo dpkg -i amdrocm7-rocm-bandwidth-test_*.deb

# Run RBT
/opt/rocm/extras-7/bin/rocm-bandwidth-test --help
```
...
sudo rpm -i --replacefiles --nodeps amdrocm7-rocm_bandwidth_test-*.rpm
...
sudo tar -xzf amdrocm7-rocm-bandwidth-test-*.tar.gz -C /opt/rocm/extras-7
...
```bash
dpkg-deb -I amdrocm*-rocm-bandwidth-test_*.deb  # Package info
dpkg-deb -c amdrocm*-rocm-bandwidth-test_*.deb  # Package contents
```

### RPM Package Verification

```bash
rpm -qip amdrocm*-rocm-bandwidth-test-*.rpm  # Package info
rpm -qlp amdrocm*-rocm-bandwidth-test-*.rpm  # Package contents
rpm -qRp amdrocm*-rocm-bandwidth-test-*.rpm  # Package dependencies
```
...
readelf -d /opt/rocm/extras-7/bin/rocm-bandwidth-test | grep RPATH

# Should show: $ORIGIN:$ORIGIN/../lib:$ORIGIN/../lib/rbt:/opt/rocm/extras-7/lib
```
...
ldd /opt/rocm/extras-7/bin/rocm-bandwidth-test
...
ls -lh build/amdrocm*-rocm-buildwidth-tests*
```
...
- [RBT Build Instructions](../README.md)
- [ROCm Documentation](https://rocm.docs.amd.com/)

## Support

For issues with:
- **RBT Build**: Open an issue in this repository
- **ROCm SDK**: See [TheRock Issues](https://github.com/ROCm/TheRock/issues)
- **Workflow**: Check GitHub Actions documentation

## License

This workflow is part of ROCm Bandwidth Test and follows the same MIT license.
